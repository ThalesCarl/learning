...
GLuint CompileShader(GLuint type, const std::string sourceCode)
{
    ...
    if(result == GL_FALSE) {
        ...
        if(type == GL_VERTEX_SHADER) {
            std::cout << "ERROR: GL_VERTEX_SHADER compilation failed with error message: " << errorMessages << std::endl;
        } else if(type == GL_FRAGMENT_SHADER) {
            std::cout << "ERROR: GL_FRAGMENT_SHADER compilation failed with error message: " << errorMessages << std::endl;
        }
